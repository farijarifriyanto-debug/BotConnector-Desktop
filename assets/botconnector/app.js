(() => {
  ...
  function createLocalHttpApi(baseOverride = '') {
    ...
    return Object.freeze({
      ...
      pairDevice: code => call('device-pair', { code }),
      connectDevice: () => call('device-connect'),
      unpairDevice: () => call('device-unpair'),
      launcherProfiles: () => call('launcher-profiles'),
      setLauncherProfileEnabled: (id, enabled) => call('launcher-set-enabled', { id, enabled }),
      startLauncherProfile: id => call('launcher-start', { id }),
      stopLauncherProfile: id => call('launcher-stop', { id }),
  ...
  async function appDeviceJson(pathname, options = {}) {
    const method = String(options.method || 'GET').toUpperCase();
    const headers = { ...(options.headers || {}) };
    if (!['GET', 'HEAD'].includes(method)) headers['x-botconnector-web'] = '1';
  ...
  function deviceButton(label, handler, className = 'secondary-action') {
    ...
    button.addEventListener('click', async () => {
      ...
      try { await handler(); } catch (error) { showToast(error.message || String(error)); }
      finally { button.disabled = false; }
    });
    return button;
  }

  async function remoteDeviceRequest(deviceId, method, params = {}) {
    return appDeviceJson(`/api/devices/${encodeURIComponent(deviceId)}/request`, {
      method: 'POST',
      body: { method, params },
    });
  }
  ...
  async function pairCurrentDevice() {
    if (!auth.checked) await refreshAuth();
    if (!auth.authenticated) {
      window.location.assign('/api/auth/start?client_id=botconnector-web');
      return;
    }
    if (!localApi && !(await connectLocalCore())) throw new Error('Local Core belum tersambung di laptop ini.');
    const pairing = await appDeviceJson('/api/devices/pair', { method: 'POST' });
    await localApi.pairDevice(pairing.code);
    showToast('Perangkat terhubung ke akun BotConnector.');
    await renderDeviceWorkspace();
  }

  async function renderDeviceWorkspace() {
    const localNode = $('#device-local-core');
    const pairNode = $('#device-pair-state');
    const bridgeNode = $('#device-bridge-state');
    const deviceList = $('#device-list');
    const profileList = $('#launcher-profile-list');
    if (!deviceList || !profileList) return;
    localNode.textContent = localApi ? 'Connected' : 'Tidak tersambung';
    deviceList.replaceChildren();
    profileList.replaceChildren();

    if (localApi) {
      const localStatus = await localApi.deviceStatus().catch(() => null);
      pairNode.textContent = localStatus?.paired ? (localStatus.deviceName || 'Terpasang') : 'Belum dipasangkan';
      bridgeNode.textContent = localStatus?.connection || 'Offline';
      const profiles = await localApi.launcherProfiles().catch(() => []);
      for (const profile of profiles) {
        ...
        const note = document.createElement('small'); note.textContent = `${profile.description} · ${profile.enabled ? 'remote diizinkan' : 'remote diblokir'}`;
        info.append(title, note);
        const actions = document.createElement('div'); actions.className = 'tool-actions';
        actions.append(deviceButton(profile.enabled ? 'Cabut izin' : 'Izinkan', async () => {
          await localApi.setLauncherProfileEnabled(profile.id, !profile.enabled);
          await renderDeviceWorkspace();
        }));
        if (profile.enabled) actions.append(deviceButton(profile.running ? 'Stop' : 'Start lokal', async () => {
          if (profile.running) await localApi.stopLauncherProfile(profile.id); else await localApi.startLauncherProfile(profile.id);
          await renderDeviceWorkspace();
        }));
        row.append(info, actions); profileList.append(row);
      }
    } else {
      pairNode.textContent = 'Local Core diperlukan';
      bridgeNode.textContent = 'Offline';
      profileList.textContent = 'Hubungkan Local Core untuk mengatur izin aplikasi pada laptop ini.';
    }

    if (!auth.checked) await refreshAuth();
    if (!auth.authenticated) {
      deviceList.textContent = 'Masuk ke akun BotConnector untuk melihat dan memasangkan perangkat.';
      return;
    }
    const payload = await appDeviceJson('/api/devices');
    const devices = Array.isArray(payload.devices) ? payload.devices : [];
    if (!devices.length) {
      deviceList.textContent = 'Belum ada perangkat pada akun ini.';
      return;
    }
    for (const device of devices) {
      const row = document.createElement('div'); row.className = 'tool-row';
      const info = document.createElement('div');
      const title = document.createElement('strong'); title.textContent = device.name || device.id;
      const gpu = Array.isArray(device.hardware?.nvidia) ? device.hardware.nvidia.map(item => item.name).filter(Boolean).join(', ') : '';
      const spec = device.hardware ? `${device.hardware.cpu || 'CPU'} · ${device.hardware.ramGb || '?'} GB RAM${gpu ? ` · ${gpu}` : ''}` : 'Hardware belum dilaporkan';
      const note = document.createElement('small'); note.textContent = `${device.online ? 'Online' : 'Offline'} · ${spec}`;
      info.append(title, note);
      const actions = document.createElement('div'); actions.className = 'tool-actions';
      if (device.online) {
        actions.append(deviceButton('Refresh hardware', async () => {
          await remoteDeviceRequest(device.id, 'hardware.get');
          await renderDeviceWorkspace();
        }));
        if (device.capabilities?.includes('launcher.start')) actions.append(deviceButton('Start Desktop Commander', async () => {
          await remoteDeviceRequest(device.id, 'launcher.start', { id: 'desktop-commander-remote' });
          showToast('Desktop Commander Remote diminta berjalan di perangkat.');
        }, 'primary-action'));
      }
      row.append(info, actions); deviceList.append(row);
    }
  }

  $('#pair-this-device')?.addEventListener('click', () => pairCurrentDevice().catch(error => showToast(error.message || String(error))));
  $('#refresh-devices')?.addEventListener('click', () => renderDeviceWorkspace().catch(error => showToast(error.message || String(error))));
  ...
  const localWorkspace = (() => {
    ...
    function setView(view) {
      ...
      $('#devices-workspace').hidden = view !== 'devices';
      ...
      const productTitle = view === 'tools' ? 'Tools' : view === 'devices' ? 'Devices' : view === 'developer' ? 'Developer' : page[0];
