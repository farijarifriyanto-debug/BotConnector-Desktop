...
```powershell
npx https://app.botconnector.id/device-cli-v0.4.5.tgz connect
```
...
```powershell
npx https://app.botconnector.id/device-cli-v0.4.5.tgz connect --code ABC123 --allow-desktop-commander
```
...
BotConnector Device CLI v0.4.5 can serve a self-contained local browser UI on
...
```powershell
npx https://app.botconnector.id/device-cli-v0.4.5.tgz offline --allow-local-ai
```
...
```powershell
npx https://app.botconnector.id/device-cli-v0.4.5.tgz connect --code <PAIR_CODE> --allow-local-ai
```

If internet connectivity drops after the session starts, the localhost UI and
already-downloaded local models continue to work. For a cold start without
network access, npm can reuse the versioned package after it has been cached:

```powershell
npx --offline https://app.botconnector.id/device-cli-v0.4.5.tgz offline --allow-local-ai
```

The local server binds only to `127.0.0.1` and protects API calls with an
in-memory random session token plus same-origin checks.
