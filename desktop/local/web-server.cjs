...
function createWebServer({ coreUrl, handlers, host = '127.0.0.1', port = 18764, onShutdown = () => {}, allowedOrigins = [], development = false }) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (pathname.startsWith(apiPrefix)) {
      ...
      }[action] || (action === 'model-download' || action === 'chat' || action === 'catalog-search' || action === 'tool-set-enabled' || action === 'tool-execute' || action === 'mcp-register' || action === 'mcp-tool-enable' || action === 'device-pair' || action === 'launcher-set-enabled' || action === 'launcher-start' || action === 'launcher-stop' ? [body] : []);
