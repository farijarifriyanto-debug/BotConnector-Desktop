...
const { LocalLauncher } = require('./local/launcher.cjs');
const { DeviceBridge } = require('./local/device-bridge.cjs');
...
function sealDeviceToken(value) {
...
function openDeviceToken(value) {
...
function registerLocalModelsApi() {
  ...
  registerLocalApi('local:device-status', () => deviceBridge.status());
  registerLocalApi('local:device-pair', ({ code } = {}) => deviceBridge.pair(code));
  registerLocalApi('local:device-connect', () => deviceBridge.connect());
  registerLocalApi('local:device-unpair', () => deviceBridge.unpair());
  registerLocalApi('local:launcher-profiles', () => launcher.list());
  registerLocalApi('local:launcher-set-enabled', ({ id, enabled } = {}) => launcher.setEnabled(id, enabled));
  registerLocalApi('local:launcher-start', ({ id } = {}) => launcher.start(id));
  registerLocalApi('local:launcher-stop', ({ id } = {}) => launcher.stop(id));
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    launcher = new LocalLauncher({ settings, emit });
    deviceBridge = new DeviceBridge({
      ...
      launcher,
      emit,
      seal: sealDeviceToken,
      open: openDeviceToken,
      cloudBase: process.env.BOTCONNECTOR_CLOUD_BASE || 'https://app.botconnector.id',
    });
    ...
    deviceBridge.connect().catch(error => emit('device:error', { message: error.message }));
    if (!localServiceMode) {
      await startCore();
