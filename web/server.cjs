...
async function readJson(req, limit = 256 * 1024) {
  ...
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw Object.assign(new Error('Payload JSON tidak valid.'), { status: 400 });
  return value;
}
...
async function resolveIdentity(req, config) {
  const sessionToken = parseCookies(req.headers.cookie)[APP_COOKIE];
  if (!sessionToken) return null;
  const resolved = await centralRequest(config, '/v1/app-auth/session/resolve', { session_token: sessionToken });
  if (!resolved.user_id || !validUuid(resolved.user_id)) return null;
  return { userId: resolved.user_id, sessionToken, resolved };
}
...
function createServer(options = {}) {
  ...
  const relay = new DeviceRelay({ appOrigin: config.appOrigin });
  const server = http.createServer(async (req, res) => {
    ...
    if (pathname === '/api/devices/pair/exchange' && req.method === 'POST') {
      try { return json(res, 200, relay.exchange(await readJson(req))); }
      catch (error) { return sendError(res, error.status || 400, 'PAIRING_FAILED', error.message || 'Pairing perangkat gagal.'); }
    }
    ...
    if (pathname === '/api/devices/pair' && req.method === 'POST') {
      ...
      let identity;
      try { identity = await resolveIdentity(req, config); }
      catch { return sendError(res, 503, 'AUTH_UNAVAILABLE', 'Layanan autentikasi tidak tersedia.'); }
      if (!identity) return sendError(res, 401, 'AUTH_REQUIRED', 'Masuk dengan akun BotConnector untuk memasangkan perangkat.');
      return json(res, 200, relay.createPairCode(identity.userId));
    }
    ...
    if (pathname === '/api/devices' && req.method === 'GET') {
      ...
      if (!identity) return sendError(res, 401, 'AUTH_REQUIRED', 'Masuk dengan akun BotConnector untuk melihat perangkat.');
      return json(res, 200, { devices: relay.list(identity.userId) });
    }

    const deviceRequest = pathname.match(/^\/api\/devices\/([^/]+)\/request$/);
    if (deviceRequest && req.method === 'POST') {
      if (!originAllowed(req, config)) return sendError(res, 403, 'ORIGIN_REJECTED', 'Permintaan tidak diizinkan.');
      let identity;
      try { identity = await resolveIdentity(req, config); }
      catch { return sendError(res, 503, 'AUTH_UNAVAILABLE', 'Layanan autentikasi tidak tersedia.'); }
      if (!identity) return sendError(res, 401, 'AUTH_REQUIRED', 'Masuk dengan akun BotConnector untuk menggunakan perangkat.');
      try {
        const body = await readJson(req);
        const result = await relay.request(identity.userId, decodeURIComponent(deviceRequest[1]), body.method, body.params || {});
        return json(res, 200, { result });
      } catch (error) {
        return sendError(res, error.status || 400, 'DEVICE_REQUEST_FAILED', error.message || 'Permintaan perangkat gagal.');
      }
    }

    const deviceRevoke = pathname.match(/^\/api\/devices\/([^/]+)\/revoke$/);
    if (deviceRevoke && req.method === 'POST') {
      if (!originAllowed(req, config)) return sendError(res, 403, 'ORIGIN_REJECTED', 'Permintaan tidak diizinkan.');
      let identity;
      try { identity = await resolveIdentity(req, config); }
      catch { return sendError(res, 503, 'AUTH_UNAVAILABLE', 'Layanan autentikasi tidak tersedia.'); }
      if (!identity) return sendError(res, 401, 'AUTH_REQUIRED', 'Masuk dengan akun BotConnector untuk mengelola perangkat.');
      const removed = relay.revoke(identity.userId, decodeURIComponent(deviceRevoke[1]));
      if (!removed) return sendError(res, 404, 'DEVICE_NOT_FOUND', 'Perangkat tidak ditemukan.');
